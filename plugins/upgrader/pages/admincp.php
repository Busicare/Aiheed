    ...
    if ($connId) {
        ...
        if ($login) {
            ...
            if (!@ftp_chdir($connId, $ftpPath)) {
                $result['message'] = "Failed: Provide the correct path to your AIheed Installation";
...
function upgrade_upload_pager($app) {
    ...
    if ($file) {
        ...
        if ($uploader->passed()) {
            ...
            $redirect_url = url('admincp/upgrade?file='.$file.'&path='.$path);
            $data['status'] = 1;
        ...
        $data['message'] = 'File was not uploaded';
    }
