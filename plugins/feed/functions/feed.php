...
function add_feed($val, $api = false) {
    ...
    $feed = db()->query("INSERT INTO `feeds` (is_poll,feeling_data,to_user_id,link_details,can_share,user_id,files,tags,entity_id,entity_type,type,type_id,type_data,photos{$music_colum_sql},video,feed_content,privacy,location,time,background) VALUES(
        '{$poll}','{$feeling}','{$to_user_id}','{$link_details}','{$can_share}','{$userid}','{$files}','{$tagsData}','{$entity_id}','{$entity_type}','{$type}','{$type_id}','{$type_data}','{$images}'{$music_value_sql},'{$video}', '{$content}', '{$privacy}', '{$location}', '{$time}', '{$background}'
    )");
    if ($feed) {
...
function get_feed_fields() {
    $sqlFields = "status,is_poll,poll_voters,feeling_data,user_id,feed_id,entity_id,entity_type,type_id,type,type_data,to_user_id,photos,video,files,feed_content,background,privacy,link_details,tags,location,can_share,shared,shared_id,shared_count,edited,time";
