...
register_hook('system.started', function($app) {
    if ($app->themeType == 'frontend' or $app->themeType == 'mobile') {
        ...
        if(config('enable-video-pop', 0)) {
            register_asset("video::js/jquery.iframetracker.js");
            register_asset("video::js/popvideo.js");
        }
    }
});
...
/*--upgraded version hook--*/


register_hook('before-render-js',function($script)
    {
        $script.=' <script>var players = []; var allIframe=[];</script>';
        return $script;
    });
