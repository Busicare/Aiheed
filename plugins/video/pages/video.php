...
function video_page_pager($app) {
    $videoId = segment(1);
    $video = $app->video;
    set_meta_tags(array('name' => get_setting("site_title", "AIheed"), 'title' => $video['title'], 'description' =>  $video['description'], 'image' =>  $video['photo_path'], 'keywords' => ''));
...
function create_pager($app) {
    ...
    if ($val) {
        ...
        if ($videoFile and config('video-upload') and config('video-encoder', 'none') != 'none') {
