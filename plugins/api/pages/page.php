...
function invite_pager($app) {
    ...
    if ($invited_id != get_userid()) {
        if(!has_liked('page', $page_id, 1, $invited_id)) {
            if(!has_page_invited($page_id, $invited_id)) {
                send_notification($invited_id, 'page.invite', $page_id);
                add_page_invite($page_id, $invited_id);
                $result = array(
                    'status' => 1,
                    'message' => lang('page::invitation-sent')
                );
            } else {
                $result['message'] = lang('page::user-already-invited');
            }
        } else {
            $result['message'] = lang('page::user-already-liked');
        }
    } else {
        $result['message'] = lang('page::cannot-invite-yourself');
    }
    return json_encode($result);
}
