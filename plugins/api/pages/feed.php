...
function save_feed_pager($app) {
    ...
    if ($user['id'] == $feed['user_id'] || is_admin() or is_moderator()) {
        $can_edit_feed =  true;
    } else {
        $result = array('edit' => false);
        $result = fire_hook('feed.edit.check', $result, array($feed));
        $can_edit_feed = $result['edit'];
    }
    $result = array(
        'status' => 0,
        'content' => '',
    ...
    if ($can_edit_feed) {
        ...
        $db = db();
        $save = $db->query("UPDATE `feeds` SET `feed_content`='{$text}', `edited`='1' WHERE `feed_id`='{$id}'");
        if ($save) {
            $result['status'] = 1;
            $result['content'] = output_text(sanitizeText($text));
