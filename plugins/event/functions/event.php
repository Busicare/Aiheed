...
function create_event($val) {
    ...
    if(isset($start_time_type)) {
        $start_time_type  = sanitizeText($start_time_type);
        $start_hour = $start_time_type == 'pm' ? $start_hour + 12 : $start_hour;
    } else {
        $start_time_type  = $start_hour < 12 ? 'am' : 'pm';
    }
    ...
    if(isset($end_time_type)) {
        $end_time_type  = sanitizeText($end_time_type);
        $end_hour = $end_time_type == 'pm' ? $end_hour + 12 : $end_hour;
    } else {
        $end_time_type  = $end_hour < 12 ? 'am' : 'pm';
    }
