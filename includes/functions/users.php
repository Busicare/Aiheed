...
function get_user_name($user = array())
{
    ...
    $user_name = trim($user_name) == '' ? $user['username'] : ucwords($user_name);
    if(!plugin_loaded('nameoption')){
        return $user_name;
    }
    if(plugin_loaded('nameoption')){
        $option= get_user_data('nameOption',$user);
        if($option==0){
            $name=$user_name;
        }
        elseif ($option==1){
            $name=get_user_data('first_name',$user);
        }
        elseif ($option==2){
            $name=get_user_data('username',$user);
        }
        else{}
        return $name;
    }
...
function get_privacy($key, $default = null, $user = null)
{
    ...
    if (isset($privacy[$key]) or in_array($key, $privacy)) return $privacy[$key];
    return $default;
}
...
function get_verification_requests($type = null)
{
    $where_clause = ($type) ? " AND type = '".$type."'" : "";
    return paginate("SELECT * FROM verification_requests WHERE ignored='0' AND (type='user' OR type='page')".$where_clause." ORDER BY time DESC");
}
...
function edit_verification_question($val) {
    ...
    $input_type = sanitizeText($val['input_type']);
    $entity = sanitizeText($val['entity']);
    db()->query("UPDATE `verification_questions` SET question = '".$question."', input_type = '".$input_type."', entity = '".$entity."'");
    return true;
}
