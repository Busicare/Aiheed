...
class Pager
{
    ...
    {
        static::$offMenus = fire_hook('more.admincp.menu',static::$offMenus,array());
        if (in_array($location, static::$offMenus)) {
            return new Menu('', '', '');
        }
        $expected = array(
...
class Uploader
{
     ...
     * @return $this
     */
    public function toDB($type = "", $typeId = "", $privacy = 1, $album = '',$uid=null)
    {
        if ($this->error) return false;
...
class CSRFProtection {
    public static function getToken() {
        ...
        if (!is_array($sessionArray)) $sessionArray = array();
